...
final class EditorV2AdapterTests: XCTestCase {
    ...
    // MARK: - Stateless render probe error propagation

    private static let viewerProbeConfigJson = #"{"initialization":{"type":"localEmpty"}}"#

    /// The probe's JSON payload is an error envelope only; a successful render
    /// is a render-elements payload, so a returned `error` object is the
    /// failure the viewer's `extractRenderError` reports to JS.
    private func probeErrorFields(_ json: String) -> [String: Any]? {
    ...
    /// The probe's success payload: a flat array of render elements.
    private func flatRenderElements(_ json: String) -> [[String: Any]]? {
        guard let data = json.data(using: .utf8),
    ...
    /// probe must report that, not a generic "could not apply content".
    func testRenderProbeReportsWhyANullDocumentCouldNotBeApplied() {
        let json = renderDocumentProbe(configJson: Self.viewerProbeConfigJson) { adapter in
            adapter.setContentJson("null")
        }

        let error = probeErrorFields(json)
        XCTAssertEqual(error?["domain"] as? String, "boundary")
        XCTAssertEqual(
            error?["message"] as? String,
            "setContentJson document is not valid JSON",
            "the probe must surface the adapter's own rejection, not a generic "
                + "contract error that names no cause"
        )
    }

    /// A well-formed JSON object that is not a document reaches the engine and
    /// is rejected there. That engine-side error has to travel all the way out
    /// of the probe, domain and message intact.
    func testRenderProbeReportsTheEnginesRejectionOfAMalformedDocument() {
        let json = renderDocumentProbe(configJson: Self.viewerProbeConfigJson) { adapter in
            adapter.setContentJson("{}")
        }

        let error = probeErrorFields(json)
        XCTAssertEqual(error?["domain"] as? String, "document")
        XCTAssertEqual(error?["code"] as? String, "DOCUMENT_INVALID")
        XCTAssertEqual(
            error?["message"] as? String,
            #"invalid JSON structure: node must have a string "type" field"#,
            "the engine's own validation message is what the viewer needs to "
                + "name the offending content"
        )
    }

    /// A successful probe returns the flat render-elements array the viewer
    /// renders — not the adapter's block-form update object, which the JS
    /// boundary rejects as an invalid payload.
    func testRenderProbeReturnsFlatRenderElementsWhenApplySucceeds() {
        let json = renderDocumentProbe(configJson: Self.viewerProbeConfigJson) { adapter in
            adapter.setContentHtml("<p>Hello</p>")
        }

        XCTAssertNil(probeErrorFields(json), "a successful probe carries no error envelope")
        let elements = flatRenderElements(json)
        XCTAssertNotNil(
            elements,
            "the probe contract is a flat array of render elements, got: \(json)"
        )
        XCTAssertEqual(elements?.first?["type"] as? String, "blockStart")
        XCTAssertTrue(
            elements?.contains { $0["text"] as? String == "Hello" } ?? false,
            "the rendered text survives flattening"
        )
    }

    // MARK: - Render element flattening

    func testRenderElementsFlattensBlocksInOrder() {
        let update = """
        {"renderBlocks":[\
        ...
        let elements = flatRenderElements(renderElementsJsonFromUpdate(update))

        XCTAssertEqual(elements?.count, 5)
        XCTAssertEqual(elements?[0]["type"] as? String, "blockStart")
        XCTAssertEqual(elements?[1]["text"] as? String, "Hello")
        XCTAssertEqual(elements?[4]["type"] as? String, "blockEnd")
    }

    func testRenderElementsPassesThroughAnAlreadyFlatPayload() {
        let update = #"{"renderElements":[{"type":"textRun","text":"Hi","marks":[]}]}"#

        let elements = flatRenderElements(renderElementsJsonFromUpdate(update))

        XCTAssertEqual(elements?.count, 1)
        XCTAssertEqual(elements?[0]["text"] as? String, "Hi")
    }

    func testRenderElementsReportsAnUpdateCarryingNoRenderPayload() {
        let error = probeErrorFields(renderElementsJsonFromUpdate(#"{"historyState":{}}"#))

        XCTAssertEqual(error?["domain"] as? String, "boundary")
        XCTAssertEqual(error?["code"] as? String, "FFI_RESULT_INVALID")
        XCTAssertEqual(
            error?["message"] as? String,
            "v2 render update carries no render payload"
        )
    }

    func testRenderElementsReportsInvalidUpdateJson() {
        let error = probeErrorFields(renderElementsJsonFromUpdate("not json"))

        XCTAssertEqual(
            error?["message"] as? String,
            "v2 render update is not valid JSON"
        )
    }
