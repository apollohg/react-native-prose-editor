...
internal object PreparedProseInstrumentation {
    ...
    private val phaseSamples = linkedMapOf<TraversalPhase, PhaseSamples>(); private val pendingCompileNanos = linkedMapOf<TraversalPhase, MutableMap<String, Long>>()
    ...
    fun beginTraversal(next: TraversalPhase) { if (BuildConfig.PREPARED_PROSE_INSTRUMENTATION) synchronized(lock) { phase = next; previousFrameNanos = 0; surfaceDrawnSinceFrame = false } }
    fun endTraversal() { if (BuildConfig.PREPARED_PROSE_INSTRUMENTATION) synchronized(lock) { phase = null; previousFrameNanos = 0; surfaceDrawnSinceFrame = false } }
    ...
    private fun resetLocked() { phase = null; phaseSamples.clear(); pendingCompileNanos.clear(); retained.clear(); duplicatePublications = 0; previousFrameNanos = 0; surfaceDrawnSinceFrame = false }
}
