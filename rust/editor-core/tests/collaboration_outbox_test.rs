...
use editor_core::collaboration_runtime::outbox::{
    set_reservation_allocation_failure_for_test, CollaborationOutbox, OutboxReservationError,
};
use editor_core::native_bridge_test_support::{self as bridge, BridgeTestOutcome, SessionOptions};
...
fn detached_session_editing_matches_direct_engine_behavior() {
    ...
    match engine
        ...
        .unwrap()
    {
        editor_core::yrs_engine::CommandPlan::Transaction(mut transaction)
        | editor_core::yrs_engine::CommandPlan::SelectionOnly(mut transaction) => {
