...
use crate::session_lifecycle_test_support::{
...
fn wait_for_state(
    handle: &crate::session_lifecycle_test_support::SessionHandle,
...
fn registry_owned_sessions_are_linearizable_and_teardown_once() {
    let _guard = crate::test_support::RegistryConcurrencyGuard::acquire();
    ...
    let destroyer = thread::spawn(move || {
        let _guard = crate::test_support::RegistryConcurrencyGuard::inherit_for_spawned_thread();
        destroy_session(id);
    });
    ...
    let repeated_destroyers: Vec<_> = (0..16)
        .map(|_| {
            thread::spawn(move || {
                let _guard =
                    crate::test_support::RegistryConcurrencyGuard::inherit_for_spawned_thread();
                destroy_session(id);
            })
        })
        .collect();
