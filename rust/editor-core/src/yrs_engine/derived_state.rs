...
mod tests {
    ...
    fn localized_text_index_deep_wide_block_discovery_has_zero_path_comparisons() {
        fn metrics(depth: usize, block_count: usize) -> (usize, usize, usize) {
            ...
            reset_localized_index_metrics_for_test();
            let state =
                initialize_test_document(&schema, json!({"type": "doc", "content": [nested]}))
    ...
    fn generic_structural_evidence_uses_current_loosened_limits_and_reuses() {
        ...
        let candidate_node_count =
            crate::editor_state::document_node_count(candidate_document.root());
        ...
        let reuse_derivations =
            compiled_test_derivations(&reuse_document, &reuse_artifact, &schema);
        let reuse_authority =
    ...
    fn active_state_retained_meter_has_exact_deep_capacity_depth_and_cardinality_bounds() {
        ...
        assert!(
            CachedActiveState::try_new(state.clone(), &exact_resources, &under_editing).is_err()
        );
        assert!(exact
        ...
        assert!(
            CachedActiveState::try_new(state.clone(), &under_resources, &exact_editing).is_err()
        );
